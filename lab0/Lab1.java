    ...
    for (int i = 0; i < 9; i++) {
      for (int j = 0; j < 19; j++) {
        if (c[i] == 7) {
          ...
          a[i][j] = Math.cos(Math.pow((Math.log(Math.abs(x[j]))*(1-(Math.pow((1/3.0/x[j])/1.0, 3))))/1.0, 3));
          
        }
        if (c[i]==3 || c[i]==5 || c[i]==13 || c[i]==19) {
          a[i][j] = Math.pow((Math.log(Math.pow(Math.abs(x[j]), 0.5))+Math.PI)/1.0, Math.sin(Math.pow(x[j], 2*x[j])));
